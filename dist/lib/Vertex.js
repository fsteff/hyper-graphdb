...
class Vertex {
    ...
    addEdgeTo(vertex, label, opts = {}) {
        if (vertex.getId() < 0)
            throw new Error('Referenced vertex has no id');
        // get feed from vertex
        if (!opts.feed && vertex.getFeed())
            opts.feed = Buffer.from(vertex.getFeed(), 'hex');
        // if the referenced vertex is in the same feed, we don't need to store that
        if (opts.feed && this.feed && opts.feed.equals(Buffer.from(this.getFeed(), 'hex')))
            opts.feed = undefined;
