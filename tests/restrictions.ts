...
import { HyperGraphDB, Vertex } from '..'
...
tape('restrict version', async t => {
    ...
    v1.addEdgeTo(v2, 'a', { restrictions: [{ rule: v2.getFeed() + '#' + ((<number>v2.getVersion()) + 1) + '/a/b', except: { rule: v2.getFeed() + '/a' } }] })
    v2.addEdgeTo(v3, 'b')
    ...
    const a = await db.queryAtVertex(v1).out('a').vertices()
    t.same(a.length, 1)
    t.same((<Vertex<SimpleGraphObject>>a[0]).getId(), v2.getId())
